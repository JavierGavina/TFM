...
if __name__ == "__main__":
    ...
    rpmap, _, [x_coords, y_coords] = get_loader()
    ...
    data_augmentation = DataAugmentation(
        path_to_generator=f"{constants.outputs.models.cgan_28}/checkpoints/c_gan499.h5")
    ...
    rpmap_ext = np.zeros(
        (rpmap.shape[0] + samples_generated_per_ap * len(constants.aps), rpmap.shape[1], rpmap.shape[2]))
    count_gen = 0
    ...
    Xtrain_generated, ytrain_generated = radiomap_generated[constants.aps].to_numpy(), radiomap_generated[
        ["Longitude", "Latitude"]].to_numpy()
    ...
    synthetic = data_augmentation(n_samples_per_label=300)
    radiomap_synthetic, _ = get_radiomap_from_rpmap(synthetic, x_coords, y_coords)
    Xtrain_synthetic, ytrain_synthetic = radiomap_synthetic[constants.aps].to_numpy(), radiomap_synthetic[
        ["Longitude", "Latitude"]].to_numpy()
    ...
    knn = GridSearchCV(KNeighborsRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                       param_grid={"n_neighbors": param_neighbors},
                       cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain, ytrain)

    rf = GridSearchCV(RandomForestRegressor(n_jobs=multiprocessing.cpu_count() - 1),
    ...
    knn_RPMAP = GridSearchCV(KNeighborsRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                             param_grid={"n_neighbors": param_neighbors},
                             cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain_RPMAP, ytrain_RPMAP)

    rf_RPMAP = GridSearchCV(RandomForestRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                            param_grid={"n_estimators": param_n_estimators},
                            cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain_RPMAP, ytrain_RPMAP)
    ...
    knn_generated = GridSearchCV(KNeighborsRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                                 param_grid={"n_neighbors": param_neighbors},
                                 cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain_generated, ytrain_generated)

    rf_generated = GridSearchCV(RandomForestRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                                param_grid={"n_estimators": param_n_estimators},
                                cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain_generated, ytrain_generated)
    ...
    knn_synthetic = GridSearchCV(KNeighborsRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                                 param_grid={"n_neighbors": param_neighbors},
                                 cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain_synthetic, ytrain_synthetic)

    rf_synthetic = GridSearchCV(RandomForestRegressor(n_jobs=multiprocessing.cpu_count() - 1),
                                param_grid={"n_estimators": param_n_estimators},
                                cv=5, n_jobs=multiprocessing.cpu_count() - 1).fit(Xtrain_synthetic, ytrain_synthetic)
